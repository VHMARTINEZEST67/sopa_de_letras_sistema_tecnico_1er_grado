...
paintCheckOk(ctx,numCols*sizeCell+10,posCheck[j]+sizeCell-5,sizeCell-2);
...
context.font = "12px "+fActi;
var widthWords = wDefs+sizeCell;var lineWord = sizeCell;lineWord = lineWord + sizeCell;for (j = 0; j < defs.length; j++) {
var index=j;lineDef=printJustify(context, defs[j] , numCols * sizeCell + 10 + sizeCell, lineWord, 10, widthWords-sizeCell);
for (i = 0; i < lineDef.length; i++) {lineWord = lineWord + 14;}lineWord=lineWord+4;j=index;}
...
function paintList(context){var title="Definición";
context.font = "12px "+fActi;var widthWords = context.measureText(title).width + 20;
var lineWord =sizeCell;var lineDef=[];context.fillStyle = colorText;context.textAlign = "left";lineWord = lineWord + sizeCell;widthWords= wDefs+sizeCell;
for (j = 0; j < defs.length; j++) {context.save();context.beginPath();context.lineWidth = 0.3;context.fillStyle = colorBack;
context.rect(numCols * sizeCell + 10, lineWord - sizeCell + 5, sizeCell - 5, sizeCell - 5);
posCheck.push(lineWord - sizeCell + 5);context.fill();context.stroke();context.restore();context.save();
var index=j;lineDef=printJustify(context, defs[j] , numCols * sizeCell + 10 + sizeCell, lineWord, 10, widthWords-sizeCell);for (i = 0; i < lineDef.length; i++) {
context.fillText(lineDef[i], numCols * sizeCell + 10 + sizeCell, lineWord-2);lineWord = lineWord + 14;}lineWord=lineWord+4;j=index;
context.restore();}
...
context.shadowColor="#999";context.shadowBlur = 20;context.shadowOffsetX = 3;context.shadowOffsetY = 3;
context.fill();context.stroke();context.fillStyle = colorText;context.font = "bold 12px " + fActi;context.fillText(title, numCols * sizeCell + 20, sizeCell - 4);context.restore();
}
